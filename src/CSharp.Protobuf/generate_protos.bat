...
set PLUGIN_EXE=%TOOLS_PATH%\windows_x64\grpc_csharp_plugin.exe
...
for %%f in ("%PROTO_PATH%\*.proto") do (
    ...
    echo Command: "%PROTOC_EXE%" --proto_path="%PROTO_PATH%" --proto_path="%WELL_KNOWN_TYPES%" --csharp_out="%OUTPUT_PATH%" --grpc_out="%OUTPUT_PATH%"  --plugin=protoc-gen-grpc="%PLUGIN_EXE%" "%%f"

    "%PROTOC_EXE%" --proto_path="%PROTO_PATH%" --proto_path="%WELL_KNOWN_TYPES%" --csharp_out="%OUTPUT_PATH%" --grpc_out="%OUTPUT_PATH%" --plugin=protoc-gen-grpc="%PLUGIN_EXE%" "%%f" 2>&1
